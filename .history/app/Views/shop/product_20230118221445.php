...
$total_jumlah_harga = [
    'name' => 'total_jumlah_harga',
    'id' => 'total_jumlah_harga',
    'value' => 0,
    'readonly' => true,
    'class' => 'form-control',
];
                        ...
                        <div class="product-details">
                            ...
                            <div class="pd-desc">
                                ...
                                <?php if ($session->get('isLoggedIn')) : ?>
                                    ...
                                    <div class="form-group">
                                        <label for="total_jumlah_harga">Pilih Kode</label>
                                        <select class="form-control" id="kode">
                                            
                                            <option value=0>Select Kode Voucher</option>
                                            <?php
                                                $db = \Config\Database::connect();
                                                $query   = $db->query('SELECT * FROM diskon');
                                                $diskons = $query->getResult();
                                                foreach ($diskons as $disc) : ?>
                                                <?php
                                                if ($disc->aktif == 1) : ?>
                                                    <option value="<?= $disc->besar_diskon; ?>"><?= $disc->kode_voucher; ?> (<?= $disc->besar_diskon; ?>%)</option>
                                                <?php endif ?>
                                            <?php endforeach ?>
                                        </select>
                                    </div>
                                    ...
                                    <div class="form-group">
                                        <?= form_label('Total Jumlah Harga', 'total_jumlah_harga') ?>
                                        <?= form_input($total_jumlah_harga) ?>
                                    </div>
                                    <div class="form-group">
                                        ...
                                        <?php foreach ($komentars as $index => $komentar) : ?>
                                            <div class="co-item">
                                                <div class="avatar-pic">
                                                    <img src="<?= base_url('img/product-single/avatar-2.png') ?>" alt="">
                                                </div>
...
<script>
    $('document').ready(function() {
        ...
        $("#service").on('change', function() {
            ...
            var total_jumlah_harga = total_harga - (total_harga * kode / 100)
            $("#total_harga").val(total_harga);
            $("#total_jumlah_harga").val(0);
        });
        ...
        $("#kode").on('change', function() {
            ...
            var total_jumlah_harga = total_harga - (total_harga * kode / 100)
            $("#total_harga").val(total_harga);
            $("#total_jumlah_harga").val(total_jumlah_harga);
        });
