...
{ 
    ...
    { 
      ...
      $diskon = $this->diskon->find($this->request->getPost('diskon'));
      $total = (intVal($data['total_harga']) - (intVal($data['total_harga']) * intVal($diskon->besar_diskon)/100));
      // dd($total);
			if(!$errors){
				$transaksiModel = new \App\Models\TransaksiModel();
        ...
        // diskon
        $data['total_harga'] = (intVal($data['total_harga']) - (intVal($data['total_harga']) * intVal($diskon->besar_diskon)/100));
